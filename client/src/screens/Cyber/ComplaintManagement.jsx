...
const ComplaintManagement = () => {
  ...
  if (complaints.length === 0 && !loading) {
    return (
      <div className="p-6 bg-gradient-to-br from-blue-900 via-purple-900 to-indigo-900 min-h-screen">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-white">Complaint Management</h1>
          <p className="text-gray-300">Overview of all cybercrime complaints</p>
        </div>
        <div className="bg-white/10 backdrop-blur-md rounded-lg shadow-xl border border-white/20 p-8 text-center">
          <p className="text-gray-300 mb-4">No complaints found. Please ensure you are logged in with admin role.</p>
          <button 
  ...
  if (loading) {
    return (
      <div className="p-6 bg-gradient-to-br from-blue-900 via-purple-900 to-indigo-900 min-h-screen flex items-center justify-center">
    ...
    <div className="p-6 bg-gradient-to-br from-blue-900 via-purple-900 to-indigo-900 min-h-screen">
      ...
      <div className="bg-white/10 backdrop-blur-md rounded-lg shadow-xl border border-white/20">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <select 
                ...
                className="border border-white/30 bg-white/10 text-white rounded px-3 py-1 text-sm backdrop-blur-sm"
              >
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-white/5">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase">Complaint ID</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase">User Name</th>
            ...
            <tbody className="divide-y divide-white/10">
              {filteredComplaints.map((complaint) => (
                <tr key={complaint.id} className="hover:bg-white/5">
                  <td className="px-6 py-4 text-sm text-blue-300">{complaint.complaint_id || `CMP-${complaint.id}`}</td>
                  <td className="px-6 py-4 text-sm text-white">{complaint.user_name}</td>
                  <td className="px-6 py-4 text-sm text-white">{complaint.category}</td>
                  <td className="px-6 py-4 text-sm text-white">{formatDate(complaint.created_at)}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 text-xs rounded-full ${getStatusColor(complaint.status)}`}>
                      {formatStatus(complaint.status)}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-sm text-blue-300 cursor-pointer hover:underline" onClick={() => openModal(complaint)}>
                    View Details
                  </td>
                </tr>
              ))}
        ...
        <div className="p-4 border-t border-white/10 flex justify-end gap-2">
          <button className="px-3 py-1 text-sm border border-white/30 text-white rounded hover:bg-white/10">
            Previous
          </button>
          <button className="px-3 py-1 text-sm border border-white/30 text-white rounded hover:bg-white/10">
            Next
